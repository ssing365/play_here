...
const RecomandPlaces = () => {
  ...
  {loading ? (
    ...
    <>
      ...
      <Carousel ref={carouselRef} interval={8000} indicators={false} controls={false}>
        {chunkedRecommendations.map((group, index) => (
          <Carousel.Item key={`carousel-${index}`}>
            ...
            <Row className="mb-4 d-flex justify-content-center">
              {group.map((place, idx) => (
                <Col
                  key={`place-${place.PLACE_ID || place.place_id || idx}`}
                  md={4}
                >
                  <div
                    className="image-container"
                    onClick={() =>
                      navigate(`/place?id=${place.PLACE_ID || place.place_id}`)
                    }
                  ...
                  >
                    ...
                    <div className="overlay">
                      <h5 className="place-name">
                        {place.PLACE_NAME || place.place_name || "이름 없음"}
                      </h5>
