        ...
        if (name === "password") {
            if (
                ...
                setErrors({
                    ...errors,
                    password:
                        "비밀번호는 8~20자 이내 영문, 숫자, 특수문자를 모두 사용해야 하며 공백을 포함할 수 없습니다.",
                });
            } else {
                const newErrors = { ...errors };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(
                `http://${remoteIp}:${port}/join/register.do`,
                formDataToSubmit,
                {
                    headers: {
    ...
    const checkUserId = async () => {
        ...
        try {
            ...
            const response = await axios.get(
                `http://localhost:8586/idcheck.do?`,
            ...
            setUserIdMessage(
                "서버 오류로 인해 아이디 중복 확인을 할 수 없습니다. 잠시 후 다시 시도해 주세요."
            );
            setIsUserIdChecked(false);
        }
    };
